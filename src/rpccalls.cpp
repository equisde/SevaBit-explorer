...
bool
rpccalls::connect_to_sevabit_daemon()
{
...
rpccalls::get_block(string const& blk_hash, block& blk, string& error_msg)
{
    ...
    {
        cerr << "get_staking_requirement: error connecting to Sevabit daemon at "
             << daemon_url << endl;
        return false;
    }
...
bool
rpccalls::get_super_node(COMMAND_RPC_GET_SUPER_NODES::response &res, const std::vector<std::string> &pubkeys)
{
    std::lock_guard<std::mutex> guard(m_daemon_rpc_mutex);
    ...
    if (!connect_to_sevabit_daemon())
    {
        cerr << "rpccalls::get_super_node_list_state: not connected to daemon" << endl;
        return result;
    }

    epee::json_rpc::request<COMMAND_RPC_GET_SUPER_NODES::request> request;
    epee::json_rpc::response<COMMAND_RPC_GET_SUPER_NODES::response, std::string> response;
    request.params.super_node_pubkeys = pubkeys;
    request.jsonrpc = "2.0";
    request.id      = epee::serialization::storage_entry(0);
    request.method  = pubkeys.empty() ? "get_all_super_nodes" : "get_super_nodes";
